...
AP4_Processor::ProcessFragments(AP4_MoovAtom*              moov, 
								...
								AP4_Array<AP4_UI64>		   *streamAtomLocations)
{
    ...
    for (AP4_List<AP4_AtomLocator>::Item* item = atoms.FirstItem();
        ...
        for (;AP4_Atom* child = moof->GetChild(AP4_ATOM_TYPE_TRAF, handlers.ItemCount());) {
			...
			AP4_Processor::FragmentHandler* handler = CreateFragmentHandler(trak, trex, traf, *m_TrackInput[trakIndex], 
				streamAtomLocations ? (*streamAtomLocations)[trakIndex] : atom_offset);
            if (handler) {
			...
			result = fragment->CreateSampleTable(moov, tfhd->GetTrackId(), m_TrackInput[trakIndex], 
				streamAtomLocations ? (*streamAtomLocations)[trakIndex] : atom_offset, mdat_payload_offset, 0, sample_table);
            if (AP4_FAILED(result)) return result;
                       ...
                       AP4_AtomFactory&  atom_factory)
{
    ...
    if (moov) {
		...
		m_TrackInput.SetItemCount(track_count);
        ...
        for (AP4_List<AP4_TrakAtom>::Item* item = trak_atoms->FirstItem(); item; item=item->GetNext()) {
			...
			m_TrackInput[index]		= trak_data_stream;
    ...
    if (moov) {
		...
		{
			result = ProcessFragments(moov, frags, mfra, sidx, sidx_position, output);
			if (AP4_FAILED(result)) return result;
...
AP4_Result
AP4_Processor::NormalizeTRAF(AP4_ContainerAtom *atom, AP4_UI32 start, AP4_UI32 end)
{
	unsigned int index(0);
	for (; AP4_Atom* child = atom->GetChild(AP4_ATOM_TYPE_TRAF, index++);) {
		AP4_ContainerAtom* traf = AP4_DYNAMIC_CAST(AP4_ContainerAtom, child);
		AP4_TfhdAtom* tfhd = AP4_DYNAMIC_CAST(AP4_TfhdAtom, traf->GetChild(AP4_ATOM_TYPE_TFHD));
		while (start < end && m_RemapedTrackIds[start] != tfhd->GetTrackId())
			;
		tfhd->SetTrackId(start+1);
	}
	return AP4_SUCCESS;
}

AP4_Result
AP4_Processor::NormalizeTREX(AP4_ContainerAtom *atom, AP4_ContainerAtom *destAtom, AP4_UI32 start, AP4_UI32 end)
{
	if (!atom)
		return AP4_ERROR_LIST_EMPTY;
	unsigned int index(0);
	for (; AP4_Atom* child = atom->GetChild(AP4_ATOM_TYPE_TREX, index++);) {
		AP4_TrexAtom* trex = AP4_DYNAMIC_CAST(AP4_TrexAtom, child);
		while (start < end && m_RemapedTrackIds[start] != trex->GetTrackId())
			;
		trex->SetTrackId(start+1);
		if (destAtom)
			destAtom->AddChild(trex->Clone());
	}
	return AP4_SUCCESS;
}
	...
	AP4_AtomFactory& atom_factory)
{
	...
	// put [moof] atoms in a separate list
	AP4_AtomParent              top_level;
	AP4_MoovAtom               *moov = NULL, *v_moov = NULL;
	AP4_AtomLocator		       *moof = NULL, *v_moof = NULL;
	AP4_UI64                    stream_offset = 0;
	for (AP4_Atom* atom = NULL;
		AP4_SUCCEEDED(atom_factory.CreateAtomFromStream(audio, atom));
		audio.Tell(stream_offset))
	{
		if (atom->GetType() == AP4_ATOM_TYPE_MDAT) {
			delete atom;
			continue;
			...
			//We don't support sidx for muxed streams
			delete atom;
			continue;
		}
		else if (atom->GetType() == AP4_ATOM_TYPE_SSIX) {
			...
			moof = new AP4_AtomLocator(atom, stream_offset);
			break;
		}
		top_level.AddChild(atom);
	}
	//Remove pssh Atoms from MOOV
	while(AP4_SUCCEEDED(moov->DeleteChild(AP4_ATOM_TYPE_PSSH,0)));
	
	//Create the Video Track
	for (AP4_Atom* atom = NULL;
		AP4_SUCCEEDED(atom_factory.CreateAtomFromStream(video, atom));
		video.Tell(stream_offset))
	{
		if (atom->GetType() == AP4_ATOM_TYPE_MOOV) {
			v_moov = AP4_DYNAMIC_CAST(AP4_MoovAtom, atom);
		} 
		else if (atom->GetType() == AP4_ATOM_TYPE_MOOF) {
			v_moof = new AP4_AtomLocator(atom, stream_offset);
			break;
		}
	}
	...
	// initialize the processor
	AP4_Result result = Initialize(top_level, audio);
	if (AP4_FAILED(result)) return result;

	// process the tracks if we have a moov atom
	AP4_Array<AP4_SampleLocator> locators;
	AP4_Cardinal                 audio_track_count = 0;
	AP4_Cardinal                 track_count = 0;
	AP4_List<AP4_TrakAtom>*      trak_atoms = NULL;
	AP4_Array<AP4_UI64>			 moof_locations;
	AP4_LargeSize                mdat_payload_size = 0;
	AP4_SampleCursor*            cursors = NULL;

	// build an array of track sample locators
	trak_atoms = &moov->GetTrakAtoms();
	audio_track_count = trak_atoms->ItemCount();
	moov->AddTrakAtoms(v_moov->GetTrakAtoms());
	track_count = trak_atoms->ItemCount();
	cursors = new AP4_SampleCursor[track_count];
	
	m_TrackHandlers.SetItemCount(track_count);
	m_TrackIds.SetItemCount(track_count);
	m_RemapedTrackIds.SetItemCount(track_count);
	m_TrackInput.SetItemCount(track_count);
	moof_locations.SetItemCount(track_count);
	for (AP4_Ordinal i = 0; i<track_count; i++) {
		m_TrackHandlers[i] = NULL;
		m_TrackIds[i] = 0;
		m_RemapedTrackIds[i] = 0;
		m_TrackInput[i] = 0;
	}
	...
	for (AP4_List<AP4_TrakAtom>::Item* item = trak_atoms->FirstItem(); item; item = item->GetNext()) {
		...
		// assign the right streams to the tracks
		AP4_ByteStream* trak_data_stream = (index < audio_track_count)?&audio:&video;

		m_TrackIds[index] = index+1;
		m_RemapedTrackIds[index] = trak->GetId();
		m_TrackInput[index] = trak_data_stream;
		//Renumberize the track
		trak->SetId(index+1);
		// create the track handler    
		m_TrackHandlers[index] = CreateTrackHandler(trak);
	...
	//Remap and join the TREX Atoms
	AP4_ContainerAtom *mvex(AP4_DYNAMIC_CAST(AP4_ContainerAtom, moov->GetChild(AP4_ATOM_TYPE_MVEX, 0))),
		*v_mvex(AP4_DYNAMIC_CAST(AP4_ContainerAtom, v_moov->GetChild(AP4_ATOM_TYPE_MVEX, 0)));
	NormalizeTREX(mvex, 0, 0, audio_track_count);
	NormalizeTREX(v_mvex, mvex, audio_track_count, track_count);

	if (!mvex && v_mvex)
		moov->AddChild(v_mvex->Clone());

	// We don't need v_moov anymore
	delete v_moov;
	v_moov = NULL;
	...
	result = AP4_SUCCESS;
	AP4_List<AP4_AtomLocator> frags;
	while ((moof || v_moof) && AP4_SUCCEEDED(result))
	{
		// Bring TrackIDs to normalized / 0-based convention
		if (v_moof)
			NormalizeTRAF(AP4_DYNAMIC_CAST(AP4_ContainerAtom, v_moof->m_Atom), audio_track_count, track_count);
		if (moof)
		{
			NormalizeTRAF(AP4_DYNAMIC_CAST(AP4_ContainerAtom, moof->m_Atom), 0, audio_track_count);
			if (v_moof)
			{
				unsigned int index(0);
				AP4_ContainerAtom *moof_a(AP4_DYNAMIC_CAST(AP4_ContainerAtom, moof->m_Atom)),
					*v_moof_a(AP4_DYNAMIC_CAST(AP4_ContainerAtom, v_moof->m_Atom));
				for (; AP4_Atom* child = v_moof_a->GetChild(AP4_ATOM_TYPE_TRAF, index++);)
					moof_a->AddChild(child->Clone());
			}
			frags.Add(moof);
		}
		else
		...
		if (moof)
		{
			moof = NULL;
			audio.Tell(stream_offset);
			if (AP4_SUCCEEDED(atom_factory.CreateAtomFromStream(audio, atom)))
			{
				if (atom->GetType() == AP4_ATOM_TYPE_MOOF)
					moof = new AP4_AtomLocator(atom, stream_offset);
				else
		...
		if (v_moof)
		{
			...
			v_moof = NULL;
			video.Tell(stream_offset);
			if (AP4_SUCCEEDED(atom_factory.CreateAtomFromStream(video, atom)))
			{
				if (atom->GetType() == AP4_ATOM_TYPE_MOOF)
					v_moof = new AP4_AtomLocator(atom, stream_offset);
				else
...
AP4_Result
AP4_Processor::Mux(	AP4_ByteStream&   audio,
					AP4_ByteStream&   video,
					AP4_ByteStream&   output,
					ProgressListener* listener,
					AP4_AtomFactory&  atom_factory)
{
	return MuxStream(audio, video, output, atom_factory);
}
